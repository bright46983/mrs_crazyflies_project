...
class Agent:
    ...
    def update_neigbors(self,other_boids):
        ...
        self.neighbor_agents = []  # Reset neighbor list
        for o_boid in other_boids:
            if o_boid is not None:
                ...
                if dis < self.neighbor_range and ang < self.neightbor_angle:
                    self.neighbor_agents.append(o_boid)

        return self.neighbor_agents
    ...
    def seperation_acc(self):
        ...
        neighbor_agents = self.neighbor_agents
        sep_acc = Point()

        if neighbor_agents != []:
            # Iterate through each neighbor boid
            for n_boid in neighbor_agents:
    ...
    def cohesion_acc(self):
        ...
        if neighbor_agents != []:
            ...
            avg_position.x /= len(neighbor_agents)
            avg_position.y /= len(neighbor_agents)
    ...
    def allignment_acc(self):
        ...
        if neighbor_agents != []:
            ...
            xvel_avg /= len(neighbor_agents)
            yvel_avg /= len(neighbor_agents)
